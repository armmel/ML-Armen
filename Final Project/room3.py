...
while True:
    ...
    for (x, y, w, h) in face_rects:
        ...
        # Draw rectangle
        rectangle_color = (0, 255, 0) if matched_names and "room3" in [access.lower() for access in known_face_access[np.argmin(distances)]] else (0, 0, 255)  # Green if recognized and has access, red otherwise
        ...
        if matched_names and "room3" in [access.lower() for access in known_face_access[np.argmin(distances)]]:
            access_text = f"{matched_names[0]} - Access Granted - Room "
            # Green if recognized and has access
            access_text_color = (0, 255, 0)
        else:
            access_text = "Unknown" if not matched_names else f"{matched_names[0]} - Access Denied - Room "
        ...
        cv2.putText(frame, access_text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, access_text_color, 1)

    cv2.imshow("Room  Face Detector", frame)
